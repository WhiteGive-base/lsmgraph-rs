...
2. 在这个干净内核上实现 engine-level K4 lifecycle、显式 LmergePolicy、
   L1+ semantic segment/filter、事件驱动 K4 maintenance scheduler，
...
| 事件驱动 K4 maintenance scheduler | 已实现，flush/read feedback/fanout 触发，默认关闭 |
| schema/tombstone-safe reopen 测试 | 已实现 |
| SNB mixed update 自动接入 K4 | 已实现为 DynamicGraphView/DeltaGraph 普通路径自然触发 |
...
| 文件 | 内容 |
|---|---|
| `src/config.rs` | 新增 `with_k4_auto_maintenance` 配置入口 |
| `src/graph.rs` | 新增 K4 lifecycle report、`K4MergePolicy`、K4 maintenance scheduler、L1+ cascade compaction |
| `src/delta/mod.rs` | 新增 K4 auto maintenance delta open/create 路径 |
| `src/dynamic_view/mod.rs` | 新增 `open_or_create_k4_delta()` |
| `src/snb/props.rs` | SNB dynamic server 改为使用 K4 delta 普通路径 |
| `src/lib.rs` | re-export K4 report、maintenance report 和 merge policy 类型 |
| `tests/engine_tests.rs` | 新增 K4 端到端、L1->L2 semantic filter、自动维护和 DeltaGraph 普通路径测试 |
...
cargo test --test engine_tests k4_auto_maintenance
cargo test --test engine_tests k4_delta_graph_normal_api_triggers_auto_maintenance
...
L1+ semantic segment/filter、事件驱动 K4 maintenance scheduler、SNB dynamic delta

...
| Aster | LSM-structured graph database design comparison | 远端无 source/image，qualitative/artifact-needed |
...
| NebulaGraph | distributed open-source graph database representative | 远端无 image/source，qualitative/image-source-pending |
...
| 系统 | 原因 | 写法 |
|---|---|---|
| Aster | 无本地 source checkout 或 Docker image | qualitative/artifact-needed |
| NebulaGraph | 无可用 image/source | image/source setup pending |
...
- 主表或 external-baseline sidebar 放 LiveGraph、Neo4j、TuGraph 的 measured rows。
...
- Aster/NebulaGraph 放 artifact attempt，不给数值。
...
> 我们把 baseline 分成两组：问题相关系统和开源图数据库。问题相关组里，LiveGraph 已完成 SF1/SF10 typed-neighbor measured baseline，并通过 edge-count 和 per-query count/hash digest；LSMGraph-style 作为内部 layout baseline；Aster 目前没有可跑 artifact，只能 qualitative。开源图数据库组里，Neo4j 和 TuGraph 已完成 SF1/SF10 并通过同一 digest gate；NebulaGraph 目前没有 image/source，只能作为 pending artifact attempt。主文不写“全面打败图数据库”，而是写在同一 typed-neighbor workload 和 correctness gate 下的 scope-limited measured comparison。

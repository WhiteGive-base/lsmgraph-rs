...
外部 baseline 现在不是只有 LiveGraph。当前进入数值表的条件是：加载同一 LDBC dense edge set，执行同一 fixed-seed sampled typed-neighbor workload，并且对每个 sampled query 做 count/hash digest correctness。按这个 gate，LiveGraph、Neo4j Community、TuGraph 都已经通过 SF1/SF10；Aster 和 NebulaGraph 仍是 artifact attempt。
...
LiveGraph 是最接近 typed-neighbor scan 的 measured external system，因为它能跑 adjacency/typed-neighbor scan。Neo4j 和 TuGraph 则把比较扩展到主流 property-graph database 实现。但三者都不是 SemL0 的同类 storage-control-plane 系统，所以必须保留 workload 和 digest gate 边界。

LSMGraph、Aster、BACH 更接近 LSM/layout 主题，适合 qualitative comparison。它们可以用来对比 multi-level CSR、LSM graph layout、adjacency/CSR transformation。SemL0 的区别不是又提出一个 layout，而是让 query signatures 作为 storage control plane 影响 pruning 和 physical rewrite。
...
ByteGraph、BG3、Galaxybase、GES、Nebula Graph 适合作 industry/system paper 的写法参考，例如 motivation、deployment framing、lessons learned。NebulaGraph 也可以作为后续 artifact attempt，但当前没有 image/source 和 loader/query/digest gate。SemL0 不应暗示自己有 production deployment 或能直接和这些系统做 production-scale apples-to-apples。
...
LiveGraph、Neo4j Community、TuGraph 都已有 SF1/SF10 measured evidence，并且 digest correctness 已通过。Verifier 固定 seed 采样 `(edge_type, src)`，用 dense edge-list truth 校验 neighbor counts/hashes，数值表中的 rows 都是 0 mismatches。Aster 和 NebulaGraph 仍要保持 artifact attempt 口径；如果要进一步加强 artifact story，可以再做独立 clean-checkout reproduction。
...
- LiveGraph、Neo4j、TuGraph 的 digest correctness 已实现并更新 external baseline 表；后续只需保持 scope-limited 表述。
- 决定 LiveGraph/Neo4j/TuGraph 数字放主 evaluation 还是放 artifact/comparison sidebar。 -->

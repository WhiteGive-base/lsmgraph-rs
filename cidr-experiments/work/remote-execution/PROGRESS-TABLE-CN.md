...
最后审计：2026-07-22 04:56（Asia/Shanghai）
...
|---|---|---|---|---|---:|
| S0 | 论文审计、目录同步、文件数、SHA、commit | `PASS_INITIAL` | 20 来源完成 REUSE/FIX/RERUN 审计；145/145 payload SHA、146 总文件通过；初始提交 `acb167e` | clean integration 当前 `4d07ba6cbcc8`；外部 store gate 收口后做最终全量 SHA、反向同步和封板 commit | 已完成 |
...
| S3 | G1 matched external / 条件式 LDBC E2E | `6_OF_6_ADAPTERS / STORE_GATES_RUNNING` | 六系统真实 adapter 全部集成；主分支 P10 33/33 + TuGraph 2/2，真实 tiny fixture 0 skip；Aster/LiveGraph final build、TuGraph importer、Neo4j/Nebula image/runtime identity 已冻结 | TuGraph 4.5.2 SF10 fresh import 正在运行；并行补 Aster wrapper 与 Neo4j fail-closed；之后依次做 Aster/Neo4j/Nebula isolated correctness | 18--24 h |
...
| P10 六系统 adapter | `PASS_ENGINEERING` | integration `4d07ba6cbcc8` | 主 P10 33/33、TuGraph 2/2；真实 tiny APIs、0 skip；未跑 SF10 timing |
...
| LiveGraph final build | `PASS_CORRECTNESS` | `/data/WorkSpace/results/P10-LIVEGRAPH-BUILD/P10-LIVEGRAPH-CORRECTNESS-BUILD-20260721T204315Z-3965a66/` | source clean；driver/lib SHA 固化；15/15 PASS；artifact 30/30 SHA；性能点 0 |
...
| TuGraph 4.5.2 frozen importer | `RUNNING_CORRECTNESS` | importer commit `e1bdf312`；run `P10-TUGRAPH-SF10-REBUILD-20260721T205301Z-2fc3a11` | tiny 新库 2 vertices/34 types/34 edges PASS；355M-edge fresh import 已以 nice=19/ionice=idle 启动；完成后 store manifest + 1700+1700 correctness；性能点 0 |
...
- [x] 完成 LiveGraph final build，并合入 Aster formal template 与 TuGraph frozen importer。
- [ ] 完成 TuGraph 4.5.2 SF10 fresh import/manifest/correctness；固化 Aster/Neo4j/Nebula SF10 store/importer gate。

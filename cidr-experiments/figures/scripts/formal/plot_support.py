#!/usr/bin/env python3
"""Strict frozen-tidy loading and run-level statistics for Figures 1--6."""
...
CONTRACT_PATH = PROJECT_ROOT / "plan" / "FIGURE-DATA-REQUIREMENTS.tsv"
...
def estimate(
    ...
    *,
    ...
    """Compatibility estimator used by the Figure 1--3 entry points."""
    ...
    for row in rows:
        ...
        values.append((row["run_id"], float(value)))
    if not values:
    ...
    if missing:
        raise DataContractError(
            f"{label}: partial missingness is ambiguous; provide all values or mark "
            "the whole group N/A"
        )
...
def estimate_field(
    ...
    *,
    ...
    """Compatibility field estimator used by Figures 1--3."""
...
def load_rows(
    ...
    *,
    ...
    """Compatibility loader for Figures 1--3 using the same strict common gate."""

    rows = load_figure_rows(
        input_path,
        figure_id=figure_id,
        experiment_ids=(experiment_id,),
        extra_fields=plot_fields,
    )
    return passing_rows(rows, label=f"{figure_id}/{experiment_id}")
